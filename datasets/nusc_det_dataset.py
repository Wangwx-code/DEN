                ...
                if self.return_depth:
                    point_depth = np.fromfile(os.path.join(
                        self.data_root, self.depth_path, f'{file_name}.bin'),
                                              dtype=np.float32,
                                              count=-1).reshape(-1, 3)
                    # point_enhanced = np.fromfile(
                    #     os.path.join(
                    #         self.data_root, "lidar_enhanced", f'{file_name}.bin'),
                    #     dtype=np.float32,
                    #     count=-1
                    # ).reshape(-1, 3)
                    # point_depth = np.concatenate((point_depth, point_enhanced), axis=0)
