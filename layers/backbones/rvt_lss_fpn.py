...
class DepthNet(nn.Module):
    ...
    def forward(self, x, pts_context, mats_dict):
        x = self.reduce_conv(x)
        ...
        pts_context = self.pts_conv(pts_context)
        depth = depth + pts_context.permute(0, 2, 1, 3).contiguous()
...
class RVTLSSFPN(BaseLSSFPN):
    ...
    def get_geometry_collapsed(self, sensor2ego_mat, intrin_mat, ida_mat, bda_mat,
        ...
        if bda_mat is not None:
            bda_mat = bda_mat.unsqueeze(1).repeat(1, num_cams, 1, 1).view(
                batch_size, num_cams, 1, 1, 1, 4, 4).half()
    ...
    def _forward_depth_net(self, feat,pts_context, mats_dict):
        return self.depth_net(feat,pts_context, mats_dict)
