...
prompts = ['rain', 'night', 'wet', 'dark', 'umbrella', 'difficult lighting']
infos = manager.list()
def generate_info(nusc, scenes, max_cam_sweeps=6, max_lidar_sweeps=10):
    po = Pool(3)

    for cur_scene in nusc.scene:
        if cur_scene['name'] not in scenes:
            continue
        if judge_weather(cur_scene) is False:
            continue
        print(cur_scene['description'])
        first_sample_token = cur_scene['first_sample_token']
        cur_sample = nusc.get('sample', first_sample_token)
        po.apply_async(func=func, args=(nusc, cur_scene, cur_sample, max_cam_sweeps, max_lidar_sweeps))
        
    po.close()
    po.join()

def judge_weather(cur_scene):
    description = cur_scene['description'].lower()
...
def func(nusc, cur_scene, cur_sample, max_cam_sweeps, max_lidar_sweeps):
    child_infos = []
    while True:
        info = dict()
        cam_datas = list()
...
def main():
    ...
    generate_info(trainval_nusc, val_scenes)
    mmcv.dump(infos, './data/nuScenes/nuscenes_infos_val_weather.pkl')
